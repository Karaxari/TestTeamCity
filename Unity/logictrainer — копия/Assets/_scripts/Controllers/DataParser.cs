...
public static class DataParser
{
    ...
    public static List<GameLevelConfiguration> ParseGameLevelConfig(string jsonData)
    {
        List<GameLevelConfiguration> gameConfig = new List<GameLevelConfiguration>();

        JSONNode gameLevelConfigJsonObj = JSONNode.Parse(jsonData);
        if (gameLevelConfigJsonObj == null)
        {
            Debug.Log("No notification. Return");
            return gameConfig;
        }
        foreach (var gameLevel in gameLevelConfigJsonObj)
        {
            GameLevelConfiguration n = JsonUtility.FromJson<GameLevelConfiguration>(gameLevel.Value.ToString());
            //n.setKey(gameLevel.Key);
        ...
        //Debug.Log("[Debug] All notifications parsed:");
        Utils.CollectionUtils.ListToString<GameLevelConfiguration>(gameConfig);
